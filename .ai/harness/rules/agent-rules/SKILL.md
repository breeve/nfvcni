...
**规则**：所有输出文件（报告、中间文件、最终文件）在文档最后需要备注原始用户输入

**说明**：
- 每个Agent在生成任何输出文件时，应在文件末尾添加原始用户输入的备注
- 使用 `---` 分隔符，后跟"原始用户输入："标签
- 这有助于后续查询和追溯用户的需求来源
- 此规则适用于所有类型的输出文件：临时文件、审查报告、最终文档等
...
原始用户输入：帮我搜索项目中认证相关的代码实现
```

**示例文件**：
```
.ai/.tmp/ai/review_report_20260101_120000.md
.ai/docs/ai/final-report.md
```
